...
def create_db():
    ...
    with sql.connect("todo.db") as con:
        cur = con.cursor()
        cur.execute(f"""
                        CREATE TABLE IF NOT EXISTS connections_couple (
                            ID INTEGER NOT NULL,
    ...
    with sql.connect("todo.db") as con:
        cur = con.cursor()
        cur.execute(f"""
                        CREATE TABLE IF NOT EXISTS requests_connections_couple (
                            ID INTEGER NOT NULL,
                            user_id INTEGER NOT NULL,
                            PRIMARY KEY(ID AUTOINCREMENT)
                        )
    ...
    with sql.connect("todo.db") as con:
        cur = con.cursor()
        cur.execute(f"""
                        CREATE TABLE IF NOT EXISTS requests_connections_groups (
                            ID INTEGER NOT NULL,
                            user_id INTEGER NOT NULL,
                            PRIMARY KEY(ID AUTOINCREMENT)
                        )
